...
import { Supply, Seed } from '../types/greenhouse';
...
export const SupplyManager: React.FC<Props> = ({ seed, onUpdate, onClose }) => {
  ...
  const handleRemoveSupply = (supplyId: string) => {
    const updatedSeed = {
      ...seed,
      supplies: seed.supplies.filter(s => s.id !== supplyId),
    };
    onUpdate(updatedSeed);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">Gerenciar Insumos - {seed.name}</h3>
